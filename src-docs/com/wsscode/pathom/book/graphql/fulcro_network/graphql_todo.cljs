(ns com.wsscode.pathom.book.graphql.fulcro-network.graphql-todo
  ...
  (rk/flex {:classes    [(if completed :.completed)
    ...
    (rk/label
      (rk/input {:type        "checkbox"
                 :checked     completed
                 :marginRight 5
                 :onChange    #(fp/transact! this [`(update-todo-item
                                                      ~{:todo/id id :todo/completed (not completed)})])})
      (str title))
  ...
  (rk/group {:marginBottom 10}
    (rk/input {:type     "text"
               ...
               :onChange #(let [value (.. % -target -value)]
                            (fm/set-value! this :todo/title value))})
...
(fp/defsc TodoSimpleDemo
  ...
  {:initial-state (fn [_] {:ui/new-todo (fp/get-initial-state NewTodo {})})
   :ident         (fn [] [::root "singleton"])
   :query         [{:ui/new-todo (fp/get-query NewTodo)}
                   {:all-todo-items (fp/get-query TodoItem)}]}
  (rk/block
    (css-injection/style-element {:component TodoSimpleDemo})
    (new-todo-ui (fp/computed
                   new-todo
                   {::on-save-todo #(fp/transact! this [`(create-todo-item ~%)])}))
    (for [todo all-todo-items]
      (todo-item (fp/computed
                   todo
                   {::on-delete-todo #(fp/transact! this [`(delete-todo-item ~todo)])})))))

(def simple-demo (fp/factory TodoSimpleDemo))

(fp/defsc Root [this {:keys [ROOT]}]
  {:query         [{:ROOT (fp/get-query TodoSimpleDemo)}]
   :initial-state {:ROOT {}}}
  (dom/div {:style {:border "1px solid grey"
                    :border-radius "3px"
                    :padding "5px"}}
    (dom/h4 "Todo List")
    (simple-demo ROOT)))

(app-types/register-app "todo-demo"
  (fn [_]
    {::app-types/root Root
     ::app-types/app  (fulcro/new-fulcro-client
                        ...
                        :networking (pfn/graphql-network "https://api.graph.cool/simple/v1/cjjkw3slu0ui40186ml4jocgk"))}))
