...
conf=getOption(conf,"triples");for(var identical=pairs.charAt(pos+1)==ch,ranges=cm.listSelections(),opening=0==pos%2,type,i$jscomp$0=0;i$jscomp$0<ranges.length;i$jscomp$0++){var range=ranges[i$jscomp$0],cur=range.head;var curType=cm.getRange(cur,Pos(cur.line,cur.ch+1));if(opening&&!range.empty())curType="surround";else if(!identical&&opening||curType!=ch)if(identical&&1<cur.ch&&0<=conf.indexOf(ch)&&cm.getRange(Pos(cur.line,cur.ch-2),cur)==ch+ch){if(2<cur.ch&&/\bstring/.test(cm.getTokenTypeAt(Pos(cur.line,
cur.ch-2))))return CodeMirror.Pass;curType="addFour"}else if(identical){range=0==cur.ch?" ":cm.getRange(Pos(cur.line,cur.ch-1),cur);if(CodeMirror.isWordChar(curType)||range==ch||CodeMirror.isWordChar(range))return CodeMirror.Pass;curType="both"}else if(opening&&(cm.getLine(cur.line).length==cur.ch||isClosingBracket(curType,pairs)||/\s/.test(curType)))curType="both";else return CodeMirror.Pass;else curType=identical&&stringStartsAfter(cm,cur)?"both":0<=conf.indexOf(ch)&&cm.getRange(cur,Pos(cur.line,
cur.ch+3))==ch+ch+ch?"skipThree":"skip";if(!type)type=curType;else if(type!=curType)return CodeMirror.Pass}var left=pos%2?pairs.charAt(pos-1):ch,right=pos%2?ch:pairs.charAt(pos+1);cm.operation(function(){if("skip"==type)cm.execCommand("goCharRight");else if("skipThree"==type)for(var i=0;3>i;i++)cm.execCommand("goCharRight");else if("surround"==type){var sels=cm.getSelections();for(i=0;i<sels.length;i++)sels[i]=left+sels[i]+right;cm.replaceSelections(sels,"around");sels=cm.listSelections().slice();
for(i=0;i<sels.length;i++){var JSCompiler_temp_const=sels,JSCompiler_temp_const$jscomp$0=i;var JSCompiler_inline_result=sels[i];var inverted=0<CodeMirror.cmpPos(JSCompiler_inline_result.anchor,JSCompiler_inline_result.head);JSCompiler_inline_result={anchor:new Pos(JSCompiler_inline_result.anchor.line,JSCompiler_inline_result.anchor.ch+(inverted?-1:1)),head:new Pos(JSCompiler_inline_result.head.line,JSCompiler_inline_result.head.ch+(inverted?1:-1))};JSCompiler_temp_const[JSCompiler_temp_const$jscomp$0]=
JSCompiler_inline_result}cm.setSelections(sels)}else"both"==type?(cm.replaceSelection(left+right,null),cm.triggerElectric(left+right),cm.execCommand("goCharLeft")):"addFour"==type&&(cm.replaceSelection(left+left+left+left,"before"),cm.execCommand("goCharRight"))})}function isClosingBracket(ch,pairs){ch=pairs.lastIndexOf(ch);return-1<ch&&1==ch%2}function charsAround(cm,pos){cm=cm.getRange(Pos(pos.line,pos.ch-1),Pos(pos.line,pos.ch+1));return 2==cm.length?cm:null}function stringStartsAfter(cm,pos){var token=
cm.getTokenAt(Pos(pos.line,pos.ch+1));return/\bstring/.test(token.type)&&token.start==pos.ch&&(0==pos.ch||!/\bstring/.test(cm.getTokenTypeAt(pos)))}var defaults={pairs:"()[]{}''\"\"",triples:"",explode:"[]{}"},Pos=CodeMirror.Pos;CodeMirror.defineOption("autoCloseBrackets",!1,function(cm,val,old){old&&old!=CodeMirror.Init&&(cm.removeKeyMap(keyMap),cm.state.closeBrackets=null);val&&(ensureBound(getOption(val,"pairs")),cm.state.closeBrackets=val,cm.addKeyMap(keyMap))});var keyMap={Backspace:function(cm){var conf=
getConfig(cm);if(!conf||cm.getOption("disableInput"))return CodeMirror.Pass;var pairs=getOption(conf,"pairs");conf=cm.listSelections();for(var i=0;i<conf.length;i++){if(!conf[i].empty())return CodeMirror.Pass;var around=charsAround(cm,conf[i].head);if(!around||0!=pairs.indexOf(around)%2)return CodeMirror.Pass}for(i=conf.length-1;0<=i;i--)pairs=conf[i].head,cm.replaceRange("",Pos(pairs.line,pairs.ch-1),Pos(pairs.line,pairs.ch+1),"+delete")},Enter:function(cm){var conf=getConfig(cm);conf=conf&&getOption(conf,
"explode");if(!conf||cm.getOption("disableInput"))return CodeMirror.Pass;for(var ranges=cm.listSelections(),i=0;i<ranges.length;i++){if(!ranges[i].empty())return CodeMirror.Pass;var around=charsAround(cm,ranges[i].head);if(!around||0!=conf.indexOf(around)%2)return CodeMirror.Pass}cm.operation(function(){var linesep=cm.lineSeparator()||"\n";cm.replaceSelection(linesep+linesep,null);cm.execCommand("goCharLeft");ranges=cm.listSelections();for(linesep=0;linesep<ranges.length;linesep++){var line=ranges[linesep].head.line;
cm.indentLine(line,null,!0);cm.indentLine(line+1,null,!0)}})}};ensureBound(defaults.pairs+"`")})}
