...
```clojure
(fp/defsc UserTextView [_ _]
  {:query [:user/name]})

(fp/defsc UserImageView [_ _]
  {:query [:user/photo-url]})

(fp/defsc UserViewsCompare [_ _]
  ;; We want to query for both, what we place here?
  {:query [{:app/current-user [???]}]})
```
...
(om/defui ^:once UserViewsCompare
  static om/IQuery
  ;; This enables the data to be reshaped to accomodate UI structure requirements.
  (query [_] [{:app/current-user [{:>/text-view (om/get-query UserTextView)}
                                  {:>/image-view (om/get-query UserImageView)}]}]))
```

The trick is to make some namespaces special, they make an edge on a graph that keeps the same context as the previous node.
