...
the inputs as a sequence, so the resolver can do some optimal implementation to handle multiple items, when this happens
...
resolve to the result, which is why you see `go` blocks in the code.  See <<AsyncParsing, Async Parsing>> for more details.  We use them in this demo so we can
...
to use the async parser in this demo is that it more easily demonstrates the n+1 issue.
...
[source,clojure]
----
include::../src-docs/com/wsscode/pathom/book/connect/batch3.cljs[]
----
...
[source,clojure]
----
include::../src-docs/com/wsscode/pathom/book/connect/batch_transform_auto_batch.cljs[]
----
...
[source,clojure]
----
{:closure-defines {com.wsscode.pathom.misc.INCLUDE_SPECS false}}
----
