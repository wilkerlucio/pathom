...
Since Pathom `2.2.0-RC5` it has alias support, this allows the output key to have a different
name than the requested key, to alias a property you must add the parameter `:pathom/as`. Example:

++++
<div x-app="interactive-parser" data-parser="tracing.demo1" class="loader">
[::root-dep
 (::root-dep {:pathom/as :renamed})
 (::root-dep {:pathom/as :other})]
</div>
<div class="space"></div>
++++
