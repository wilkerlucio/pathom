...
`::pc/attr-output-in` - a set containg the symbols of the resolvers where this attribute
...
The map entry value is a set containg the resolvers available to traverse that path.
...
as a leaf, this means the data is wrong or the expecification is not complete enough and is
