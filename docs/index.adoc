...
. Outputs - A query-like notation that gives the "pattern" of the part of the query the resolver is able to resolve. This is typically a list of attributes/joins, where joins typically include a simple sub-query.
...
Once you have your maps ready, you can register them using the connect `register` function into the index.
...
(-> {}
    ...
    ; you can also send collections to register many mutations at once
    (pc/register [some-resolver send-message-mutation])

    ; collections will be recursively processed, so this is valid too:
    (pc/register [some-resolver [send-message-mutation]]))

; in the end the index will have the combined information from all the resolvers and mutations
----

If you are a library author, consider defining each resolver/mutation as its own symbol
and then create another symbol that is a vector combining your features, this way you
make easy for your users to just get the vector but still allow them to cherry-pick
which operations they want to pull if they don't want all of it.
...
make the installation, here is an example:
...
And that's it, the resolvers will be registered right after the parser gets defined.
...
running inside core.async `go` blocks. However, in case of CLJS most IO is done asyncronously, making
this a non-issue. But if you are in Java environment and doing blocking IO, this means
...
can't wait now, Pathom provides a thread pool helper so you can tell the engine to
...
is capable of detecting attribute dependencies, execute multiple requests in parallel
and coordinate the return, including back tracking for secondary paths. Here is how it works:

Getting back to the basic idea of `Connect`, we expand information from a context. To illustrate
...
Note that we have two resolvers which depend on a `:movie/id` and another one which depends on `:movie/title`.
...
reach the attribute considering the data it has now, it does so by recursively iterating over
the `::pc/index-oir` until it reaches some available dependency or gives up because there is
no possible path.

In most cases (specially for small apis) there will be only a single path and this is the case
...
available data to the attribute requested, this is the base plan.
...
you put `:movie/title-prefixed` at the start, you will this attribute as being responsible
for the fetching the title and itself.
...
`:movie/title`: This attribute is not in an entity, so it will create the plan to call `movie-details`
from this plan we can also compute all the attributes we will incorporate in the call chain
(by combining the outs of all the resolvers in the path) and then we store this information as a waiting list.
The waiting list in this case is: `[:movie/id :movie/title :movie/releast-date]`. The processing of
attributes continues in parallel while the resolver is called.

`:movie/release-date`: This attribute is not in entity, but it is in the waiting list, so
the parser will ignore it for now and skip to process the next one.

`:movie/rating`: This attribute is not in entity or in the waiting list, so we can
call the resolver for it immediately and the plan output (`[:movie/rating]`) is appended to the
waiting list.

`:movie/title-prefixed`: Like the rating attribute, this is not in entity or waiting, so we compute
...
the resolver, but with two listeners on the promise channel (and any posterior cache hit would
...
. Once a resolver is called, its execution time is recorded and updated in the map using the formula: new-value = (old-value + last-time) * 0.5
...
If you'd like to plug-in your own sorting plan, you can set the key `::pc/sort-plan` in your
...
This reader leverages some tecniques which were developed during the
...
When an ident query reaches this reader it will check the index to see if the ident key is present in the indexed
...
relationships on attributes. `Connect` will also look up the index in the environment on the
...
`resolver-data` is any relevant information that you want to add about that resolver. Any
key that you adding during `pc/add` will end up in this map. Also `Connect` will
...
This `index-oir` stands for `output -> input -> resolver`. It's the index used for the `Connect`
...
If we start with an ident, our initial context is `{:id 123}`. This time we have the `:id`, so
...
Added in Pathom `2.2.13`, this index contains detailed information about the system attributes
and its connections, this index is intended to be used by various tools to provide extra
...
`::pc/attr-provides` - A map telling what attributes can be provided given the current one
as a base, this only considers direct outputs (that can be reached in a single resolver call).
For each map entry, the key can either be a keyword (in case the output is provided at
...
For each map entry, the key can either be a set (in case the input is provided at
...
IMPORTANT: An attribute should never be a leaf and a branch at the same time! By being a branch
it means that attribute value is expected to be a map or a vector of maps. If later it appears
as a leaf, this means the data is wrong or the specification is not complete enough and this is
a sign something is mis-matched.
...
The `idents` index contains information about which single attributes can be used to access
...
grey header to collapse a group. Try it out in the demo above.
...
In the counters section here, there is an explanation for some non-obvious counters:

* Globals count: The number of attributes accessible that don't depend on any data
* Idents count: Number of attributes which by themselves can provide more data
...
point to effective "hubs" at the center of your data.

=== Attribute View

When you navigate to an attribute you will be on the attribute view. This view can tell
...
When you highlight some element, you can see a label for it on the top left corner. The
...
So far, every attribute we saw was a one to one attribute connection but in Pathom we
...
As you hover you mouse over the resolver, the link will show up below the attribute.
...
One common issue with the index explorer is the fact that resolvers include functions and may
include other things which are not possible to encode with Transit by default. We suggest
...
And this is how to do it in ClojureScript:
...
If you want to obtain a person's name and the street name from their address you might write this:

```
[:person/name {:person/address [:address/street]}]
```

Here we imagine that the underlying database has some kind of normalization that needs to be traversed in order to satisfy the address data.
...
elements of a query cannot be fulfilled as they are rooted in a context.  This applies to joins as well (e.g. what is the current person's
address?), but once you've resolved the context of the *root* for some graph query then joins simply describe navigation
...
with some context (e.g. "Sam").  When it finds a join it processes the sub-query against a new context (e.g. Sam's address)
to give the result:
...
by resolving a specific entity, or is the entity (or entities if to-many) that has been reached by processing the joins
of the query.
...
`[:name :age]`. If you ask an `om.next` parser to read this the reader function will be called twice; once for `:name` and another one for `:age`. Note that in the case of joins, the parser will only be called for the join entry but not for its children (not automatically). For example: given a query `[:name :age {:parent [:name :gender]}]`, the reader function will be called 3 times, one for `:name`, one for `:age` and one for `:parent`. When reading `:parent`, your reader code is responsible for checking that it has a children query and do a recursive call (or anything else you want to do to handle this join). In this guide, we are going to see many ways to implement those readers.
...
These are quite simply functions which receive the `env` and resolve the read. More than one reader can exist in a chain and the special return value `::p/continue` allows a reader to indicate it cannot resolve the given property (to continue processing the chain). And returning any value (including `nil`) you've resolved the property to that value.
...
Not all things need to be computed.  Very often the current context will already have attributes which were read during
...
* If the attribute requested exists in the current parsing context (with any value, even nil), it returns that.
...
and a vector of data at that join key in the context, then it will attempt to fulfill the sub-query of the join.
...
An entity to `pathom` is the graph node that is tracked as the current context and from which information (attributes and graph edges to other entities) can be derived.  The current entity needs to be "map-like" i.e. it should work with all normal map-related functions like `get`, `contains?`, etc.
...
The `map-reader` just has the additional ability to understand how to walk a map that has a tree shape which already "fits" our query:
...
The sub-query for a join is in the `:query` of the environment. Essentially it is a recursive step where we run the parser on the sub-query while replacing the "current entity":
...
. Using the invented join property in a normal join. This allows for a sub-query that constrains the data returned (from the actor in this case).
. Using the `*` in a query, which returns all "known" attributes of the "current contextual" entity.
. Using an additional (non-joined) `:character/voice` with `*` "adds in" that additional information. When a property is queried for that is processed via `p/join` then the entire entity will be returned even though there is no sub-query.
...
Union queries allow us to handle edges that lead to heterogeneous nodes. For example a to-many relation for media that could result in a book or movie. Following such an edge requires that we have a different *sub-query* depending on what we *actually* find in the database.
...
NOTE: Before Pathom version 2.2.0 you had to include the `p/request-cache` plugin into your plugin list. However, since 2.2.0 this is no longer necessary as
it's always available

As your queries grow, there are more and more optimizations that you can do avoid unnecessary IO or heavy computations. Here we are going to talk about a `request cache`, which is a fancy name for an atom which is initialized on every query and stays on the environment so you can share the cache across nodes. Let's see how we can use that to speed up our query processing:
...
Pathom allows a parser to have a collection of plugins which modify its behavior. `Plugin` is a top-level option while creating the parser and the value is a vector of plugins:
...
called it. In cases where there are specific things which you always wish to include in the environment we can instead
...
and our parser needs to be shard-aware. Let's write a plugin such that anytime it sees a `:shard`
...
Flattening your data makes it much more convenient to use because it increases the connection
...
Then we add a component to render the group header.
...
To fill in the `???`, the trick is to make some namespaces special as they make an edge on
...
the flattening idea, you get the best of both worlds where one entity can hold as many
...
The new tracer works as an event stream, you can inject log events at any time. Events
might have a duration or not (even for events with start and finish, they are recorded as
...
It's good to know how your queries are performing, and breaking it down by nodes is an excellent way to reason about how your queries are doing. Pathom provides a plugin to make this measurement easy to do:
...
Looking at the profiling results, you see the query values and at the edges is the `ms` time taken to process that node. When the node has children, a `::p.profile/self` indicates the time for the node itself (including children).

If you like to print a flame-graph of this output, you can use some d3 libraries on the web. I recommend the [d3 flame graph from spierman](https://github.com/spiermar/d3-flame-graph). Pathom has an in-built function to convert the profile data to the format accepted by that library:
...
As you go deep in your parser `pathom` track record of the current path taken, it's available at `::p/path` at any time. It's a vector containing the current path from the root, the current main usage for it is regarding error reporting and profiling.
...
parallel parser, which is async as well.
...
values. This allows for the creation of parsers which do network requests or any other async operation.
...
The core plug-ins work normally with the async parser, so error and profiling will work as expected.
...
In the JS world, most of the current async responses comes as promises and you can use the `<!p` macro to read from promises
...
The parameter will be removed from the list, and the alias will be created. In case you don't
like the syntax, since this is just Clojure data we can make some helper function around:
...
entity. An ident takes the form of a vector with two elements, the first element will
determine the "identity type", which is always a keyword and on the second element you have the "identity value", which can be
any EDN value.

Some known applications of EQL idents:

* Datomic uses it as link:http://blog.datomic.com/2014/02/datomic-lookup-refs.html[lookup refs]
...
In some cases one param is not enough, so there is a supported `And` notation:
...
But to be honest, that just looks bad. If you need more than one param, it's better just 
not to an ident for direct translation. However, this can be useful if you need a quick
...
understand what a translation means:
...
There are two main ways to use Pathom + GraphQL + Fulcro:
...
recommend new implementations which doesn't transform the names in any way by default but
...
adding a new todo item looks like this:
...
into the GraphQL query shown previously.  This is the purpose of the "Ident Map",  it is a map whose top-level keys are GraphQL
...
code and so anyone can use the example un-edited. In Chrome, you can set this via
...
Next, we need to create a parser. This will essentially be like this:
...
That's OK, we can define a resolver for a root-level Connect property which can pre-establish some repositories
into our context!
...
Remember to add this resolver definition before the parser, then to add this resolver
to the Connect system; do that by updating the call to the `connect-plugin`, here is the updated
parser:
...
The queries which are supported "out of the box" are those queries which follow the allowed shape of the documented
...
parameter and the fact that EDN idents only really have a single spot for one bit of data beyond
...
Now, remember that the following query:
...
What we're going to do is to allow parameters to make up the difference. If you are unfamiliar with them,
you just surround the element of the query in a list and add a map of params, like so:
...
handling and book-keeping on the query result. The simplest way to customize
query results is to pass in a custom `mung` and `demung` functions. These can be
...
and see the GraphQL equivalent being generated on the right.
