...
resolve to the result, which is why you see `go` blocks in the code.  See <<Async Parsing, AsyncParsing>> for more details.  We use them in this example so we can
...
for creation of parsers that do network requests or any other async operation. The async parser is still a *serial*
parser, and it will have the same flow characteristics of the regular parser, so the order or resolution is
preserved.
