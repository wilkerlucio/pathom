...
Since version `2.2` Pathom adds support to describe resolvers as pure maps and `register` those resolvers in
...
when the plugin is used. To do that your plugin must provide the `::pc/resolvers` key
...
IMPORTANT: Since version `2.2.0-beta11` the pathon connect <<connect-ident-reader,ident-reader>> got support for extra context using the parameter `:pathom/context`, so you don't have to do any of the setup described here, but I think it still interesting to demonstrate how to write a custom ident reader, so this section will still live here.
