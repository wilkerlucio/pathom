...
You can validate the query by running it against the specs, use the demo below to try it:
...
Async is now a first class feature of Pathom. Async parsers allow for client-side parsers that do remote calls to web
services, allowing for a full client implementation of parsers.

++++
<div x-app="async-parser"></div>
<div class="space"></div>
++++
...
[source,clojure]
----
include::../src/com/wsscode/pathom/book/connect/index_oir_example.cljc[]
----
