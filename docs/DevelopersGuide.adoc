...
`:com.wsscode.pathom.core/reader` can be used to supply a reader function for the parser to use.  The `reader` is
simply a function that is asked "read *this* particular thing (e.g. `:person/name`) in the current environment
from whatever data source can supply it".
