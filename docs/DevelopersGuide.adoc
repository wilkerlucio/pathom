...
The `::pc/args` is currently a non-op, but in the future it can be used to validate the mutation input, it's format
...
[{(create-user {:user/name "Rick Sanches" :user/email "rick@morty.com"})
...
[{(create-user {:user/id "TMP_ID" :user/name "Rick Sanches" :user/email "rick@morty.com"})
  [:user/id :user/name :user/created-at]}]
</div>
<div class="space"></div>
++++
