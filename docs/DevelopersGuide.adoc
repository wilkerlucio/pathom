...
This source for this book is at https://github.com/wilkerlucio/pathom/docs/DevelopersGuide.adoc.  Feel free to send a PR
...
(defresolver `person-name-resolver
  ...
  (fn [{:keys [database] :as env} {:keys [person/id]}]
    (let [{:keys [first-name last-name]} (my-database/get-person database id)]
...
This is what makes `Connect` powerful: by leveraging the index containing the attribute relationships you can focus on
writing just the primary `edges` of the graph, and then all possible paths can be walked automatically. You can read
more about how this works in the Index page.
...
"sleep" in a Javascript environment to mimic overhead in processing.
...
The main functions for dealing with translating to/from GraphQL are:

`pg/query->graphql` - Converts and EDN graph query to a GraphQL string.
`pfn/graphql-response-parser` - A parser that can take a GraphQL response and an EDN query to generate a proper response
tree.

The two are combined together for you as a Fulcro Remote in `pfn/graphql-network`, so you can easily add plain GraphQL
...
FIXME: DEMO BROKEN!!! (in workspaces too...add fails, setTimeout on keystrokes is hosed...etc.)

++++
<div x-app="todo-demo"></div>
++++
...
[source,clojure]
----
include::../src-docs/com/wsscode/pathom/book/graphql/fulcro-network/graphql-todo.cljs[]
----
...
First, you need to declare a place to store the indexes, a way to build your own resolvers, and (optionally) a way
to inject your <<ConnectMutations,own custom mutations>> into the Connect processing system.
...
`::pcg/resolver`:: A symbol. Required, but used primarily for debugging.
...
=== Connect-Based Mutations (Advanced) [[ConnectMutations]]

It is possible that you might want to define a mutation that is *not* on the GraphQL API, but which does some
alternative remote operation.  This is what the `defmutation` in our earlier setup was about (not to be confused
with Fulcro's `defmutation` macro).
...
The `fn` of the mutation can return a value (sync) or a channel (async).  This means that the custom mutation
...
Of course, if you're using Fulcro or Om Next, then you'll also have to make sure they're OK with the mutation symbolically
(e.g. define a `fm/defmutation` as well).

=== Multiple Input Entry Points (Advanced) [[MultiInputIdents]]
...
. Create an ident reader that can put parameter data into the parsing context.
. Add the custom ident reader to the Connect readers.
. Pass the additional data via parameters (typically write a helper to make it pretty).

==== Custom Ident Reader

An ident reader is just what it sounds like: it's a reader that, when called on an ident, does some
processing for it.  The default ident reader in connect just detects the ident and does a join. It looks like this:
...
where we've chosen `:entry-point-context` to be the parameter name that will hold the extra stuff we want to
push into the parsing context.
...
In our case we want the ident reader to look for the `:entry-point-context` parameter and plug it into the parsing context.
