...
(def parser
  (p/parser {::p/plugins
             [(p/env-plugin
                {::p/reader             [p/map-reader pc/all-readers]
...
NOTE: Resolvers *must* always return a map with at least one key. We'll cover how to indicate "not found" in a bit.
  ...
  (fn [{:keys [database] :as env} {:keys [person/id]}]
    (let [person (my-database/get-person database id)]
      {:person/age (:age person)
       :person/name (str (:first-name person) " " (:last-name person))})))
```
...
At the moment we only know how to start a graph query from a "global" resolver.

Another significant entry point for the graph are idents. Since an ident is really a lookup for a distinct entity, it is
trivial to start a graph query at one. Examples are things like "product by id" and "user by email". It turns out that
single-input resolvers automatically define how to start a graph query at an ident!

For example we have a resolver to get the brand from the product id: so `:product/id` can be used to find that.
Also the `:product/brand-id` can be realized from `:product/brand`. Thus, the following ident-join queries already work
without having to define anything else:
...
By using idents as the "attribute" of the join we are providing the initial context. An ident join on `[:product/id 1]`
will pre-fill the contextual entity with `{:product/id 1}`, and once that initial context is there the rest of the relational
resolvers can be tried to deal with the rest of the query!
...
= GraphQL Integration #TODO [[GraphQL]]
