...
In connect, unions have a default branching implementation that tries to find the union
key in the current entity, when its found, that branch is taken.
...
When deciding the branch, the presence of the key attribute on each entry is what decides
the branch that will be taken.

Note that the attribute check will be tried in the order that the map gets scanned, recent
versions of Clojure make it consistent with the order you wrote than in the map declaration.

So in case you have the chance of records hitting multiple attributes, put the ones with
the highest priority at the top of the map.
...
[source,clojure]
----
include::example$com/wsscode/pathom/book/connect/unions.cljs[]
----
