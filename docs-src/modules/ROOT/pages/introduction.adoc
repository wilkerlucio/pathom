...
The parser receives the whole query to process, it will iterate over each attribute calling
...
It is common to have a chain of readers (as in the photo and demo), for the chain case
it will try each reader in order, a reader can respond with a value or signal the engine
that it can't process the key, in this case the next reader will be attempted.

=== Resolvers

Resolvers are what you will write most while using Pathom, they are small blocks of
functions that do a step to process data.
